...
class Linear(object):
    ...
    def forward(x, w, b):
        ...
        #pass
        flat=torch.nn.Flatten()(x)
        #flat = x.view(x.shape[0],-1)
        out = torch.matmul(flat, w) + b
        ######################################################################
        #                        END OF YOUR CODE                            #
        ######################################################################
        cache = (x, w, b)
    ...
    def backward(dout, cache):
        ...
        # Replace "pass" statement with your code
        #pass
        dflat = torch.matmul(dout, w.t())
        dx = dflat.view(x.size())
...
class Linear_ReLU(object):
    ...
    def forward(x, w, b):
        ...
        # Replace "pass" statement with your code
        #pass
        out1, lin_cache = Linear.forward(x, w, b)
        out, relu_cache = ReLU.forward(out1)
        cache = (lin_cache, relu_cache)
        ######################################################################
        #                        END OF YOUR CODE                            #
        ######################################################################
        return out, cache
    ...
    def backward(dout, cache):
        ...
        # Replace "pass" statement with your code
        #pass
        lin_cache, relu_cache = cache
        drelu_out = ReLU.backward(dout, relu_cache)
        dx, dw, db = Linear.backward(drelu_out, lin_cache)
        ######################################################################
        #                END OF YOUR CODE                                    #
        ######################################################################
        return dx, dw, db
...
def softmax_loss(x, y):
    ...
    # Replace "pass" statement with your code
    N = x.shape[0]
    exponents = torch.exp(x - torch.max(x, dim=1, keepdim=True)[0])  # Subtracting the max for numerical stability
    probabilities = exponents / torch.sum(exponents, dim=1, keepdim=True)
    loss = -torch.sum(torch.log(probabilities[torch.arange(N), y])) / N
    dx = probabilities.clone()
...
class TwoLayerNet(object):
    ...
    def __init__(self, input_dim=3*32*32, hidden_dim=100, num_classes=10,
        ...
        #pass
        self.dtype = dtype
        self.params['W1'] = (torch.randn(input_dim, hidden_dim,dtype=self.dtype) * weight_scale).to(device)
        self.params['b1'] = torch.zeros(hidden_dim,dtype=self.dtype).to(device)
        self.params['W2'] = (torch.randn(hidden_dim, num_classes,dtype=self.dtype) * weight_scale).to(device)
        self.params['b2'] = torch.zeros(num_classes,dtype=self.dtype).to(device)
        ...
        self.to(device)
        def to(self, device):
        #Move the parameters and buffers to device.
    ...
    def loss(self, X, y=None):
        ...
        lr_h, cach = Linear_ReLU.forward(X, self.params['W1'], self.params['b1'])
        #cache['linear1' ] = cach[0]
        #cache['relu1' ] = cach[1]
        #if self.use_dropout:
        #    lr_h, dropout_cache['dropout'] = dropout.forward(lr_h, self.dropout_param)
        scores, cach2 = Linear.forward(lr_h, self.params['W2'],self.params['b2'])
        ...
        # Backward pass
        dout, grads['W2'], grads['b2'] = Linear.backward(dout, cach2)
        # Add regularization to the gradients
        grads['W2'] += 2*self.reg * self.params['W2']

        #if self.use_dropout:
        #    dout = dropout.backward(dout, dropout_cache['dropout'])
        dout, grads['W1'], grads['b1'] = Linear_ReLU.backward(dout, cach)
        # Add regularization to the gradients
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for i in range(1, self.num_layers + 1):
            if i == 1:
                self.params['W' + str(i)] = (torch.randn(input_dim, hidden_dims[i - 1], dtype=self.dtype) * weight_scale).to(device)
                self.params['b' + str(i)] = (torch.zeros(hidden_dims[i - 1], dtype=self.dtype)).to(device)
            elif i == self.num_layers:
                self.params['W' + str(i)] = (torch.randn(hidden_dims[i - 2], num_classes, dtype=self.dtype) * weight_scale).to(device)
                self.params['b' + str(i)] = (torch.zeros(num_classes, dtype=self.dtype)).to(device)
            else:
                self.params['W' + str(i)] = (torch.randn(hidden_dims[i - 2], hidden_dims[i - 1], dtype=self.dtype) * weight_scale).to(device)
                self.params['b' + str(i)] = (torch.zeros(hidden_dims[i - 1], dtype=self.dtype)).to(device)
    ...
    def loss(self, X, y=None):
        ...
        for i in range(1, self.num_layers + 1):
            if i == 1:
                lr_h, cache[str(i)] = Linear_ReLU.forward(X, self.params['W' + str(i)],self.params['b' + str(i)])
                #cache['linear' + str(i)]=cach[0]
                #cache['relu' + str(i)]=cach[1]
                if self.use_dropout:
                    lr_h, dropout_cache['dropout' + str(i)] = Dropout.forward(lr_h, self.dropout_param)
            elif i == self.num_layers:
                scores, cache[str(i)] = Linear.forward(lr_h, self.params['W' + str(i)],
                                                                  self.params['b' + str(i)])
            else:
                lr_h, cache[str(i)] = Linear_ReLU.forward(lr_h, self.params['W' + str(i)], self.params['b' + str(i)])
                #cache['linear' + str(i)] = cach[0]
                #cache['relu' + str(i)] = cach[1]
                if self.use_dropout:
                    lr_h, dropout_cache['dropout' + str(i)] = Dropout.forward(lr_h, self.dropout_param)
        ...
        for i in range(self.num_layers, 0, -1):
            if i == self.num_layers:
                dout, grads['W' + str(i)], grads['b' + str(i)] = Linear.backward(dout, cache[ str(i)])
            else:
                if self.use_dropout:
                    dout = Dropout.backward(dout, dropout_cache['dropout' + str(i)])
                #dout = relu_backward(dout, cache['relu' + str(i)])
                dout, grads['W' + str(i)], grads['b' + str(i)] = Linear_ReLU.backward(dout, cache[ str(i)])
...
def create_solver_instance(data_dict, dtype, device):
    ...
    from libs import Solver
    #solver = Solver(model, data_dict, num_epochs=50, batch_size=64, print_every=500, device=device)
    solver = Solver(model, data_dict,
                         optim_config={'learning_rate': 1e-1, },
                         num_epochs=10, batch_size=100, print_every=1000, device=device)
    # Train the model until it achieves at least 50% accuracy on the validation set
    #solver.train()
